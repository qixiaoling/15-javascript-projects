...
function displayMenuItems(originalDoc){
  let displayMenu = originalDoc.map((ori)=>{
    return `<article class="menu-item">
        <img src=${ori.img} alt=${ori.title} class="photo" />
        <div class="item-info">
          <header>
            <h4>${ori.title}</h4>
            <h4 class="price">${ori.price}</h4>
          </header>
          <p class="item-text">
            ${ori.desc}
          </p>
 ...
 //After the dynamic buttons are injected in HTML, we can now choose those buttons
  ...
  filterBtns.forEach(function(btn){
    btn.addEventListener("click", function(e){
      ...
      const oriCategory = menu.filter(function(oriItem){
        if(oriItem.category === chozenCategory){
          return oriItem;
        }
      });
